...
public class Stream implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command command, String string, String[] args) {	
		...
		String path = "commands.stream";
		int cooldown = config.getInt("configuration.stream.cooldown");
		...
		List<String> message = config.getStringList("configuration.stream.message");
		...
		if(p.hasPermission("mediacommands.command.stream") && p.hasPermission("mediacommands.*")) {
		...
		if(args[0].contains(".")) {
			for(int i = 0;i <message.size();i++) {
				String msg = message.get(i).replaceAll("%player%", p.getName()).replace("<stream_url>", args[0]);
		...
		p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&cUse: &f/stream <url>"));
		return true;
		}
